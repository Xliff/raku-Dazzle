...
class Dazzle::Application is GTK::Application {
  ...
  submethod BUILD (:$dzl-application) {
  ...
  multi method new (DzlApplicationAncestry $dzl-application, :$raw = True) {
  ...
  multi method new (Str() $application_id, Int() $flags = 0) {
    my GApplicationFlags $f = $flags;

    my $dzl-application = dzl_application_new($application_id, $f);

    $dzl-application ?? self.bless( :$dzl-application ) !! Nil;
  }
