...
role Dazzle::Additions::Widget {
  ...
  method replaceStyleClass ($remove, $add)  {
    self.remove_style_class($remove);
    self.add_style_class($add);
  }

  method hasStyleClass ($class) {
    self.style-context.has-class($class);
  }

}


role Dazzle::Additions::ListStore {
...
role Dazzle::Additions::TextBuffer {
